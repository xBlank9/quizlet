...
# --- In-memory storage ---
# NEW: Nested dictionary to store quizzes by category
# Example: { "Endocrine": { "Pituitary Gland": [questions] } }
quizzes = {}
user_sessions = {}
...
async def show_main_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, is_edit: bool = False):
    message_object = update.message if not is_edit else update.callback_query.message
    if not quizzes:
    ...
    text = "**أهلاً بك!** 👋\n\nالرجاء اختيار فئة الاختبارات الرئيسية:"
...
async def category_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if category not in quizzes: await query.edit_message_text("عذرًا، هذه الفئة لم تعد متاحة."); return
...
async def quiz_info_page_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    keyboard = [[InlineKeyboardButton("🚀 ابدأ الاختبار", callback_data=f"startquiz_{category}|{quiz_name}")],
                [InlineKeyboardButton("🔙 عودة لقائمة الفئة", callback_data=f"category_{category}")]]
...
# --- (The rest of the code: send_poll_question, handle_poll_answer, end_quiz, cancel, main is the same as the previous final version) ---
